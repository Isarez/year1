...
test('คลังเฟอร์นิเจอร์: ข้อมูลทุกชิ้นครบและไม่ซ้ำ', async ({ page }) => {
  ...
  const bad = meta.filter(m =>
    ...
    !['sit', 'sleep', 'bounce', 'spin', 'toggle', 'slide', 'pethouse', 'music',
      'basket', 'tv', 'tank', 'read', 'wash'].includes(m.action) ||
    /* slide = ปีนขึ้นแล้วลื่นลง (สไลเดอร์) · pethouse = สั่งสัตว์เลี้ยงเข้า/ออกบ้านสัตว์ · music = เครื่องดนตรี (เฟส 9)
       ...
       🗑️ `tank` เคยถูกถอดออก 2026-08-17 แล้ว **กลับมาอีกครั้งในเฟส 17** (ให้อาหารปลาในตู้เป็น
          action ในโลก 3D จริง ไม่ใช่ popup) พร้อมกับ `tv` `read` `wash`
          ⇒ ลิสต์นี้ตกหล่นมาตั้งแต่เฟส 17 ทำให้เทสนี้แดงเงียบๆ (เจอตอนรันชุดเต็ม 2026-08-20)
