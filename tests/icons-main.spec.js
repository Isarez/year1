...
/* 🔒 IM8 — API เขียนข้อมูลสำหรับเทสต้องผูกกับสวิตช์ `DEV_API_ENABLED` จุดเดียว
   (ผู้ใช้ถาม 2026-08-23 เรื่องแก้ localStorage โกงค่า → เลือกทางถอด API โกงออกจาก production)

   ⚠ เทสนี้ **ปรับตามสถานะสวิตช์เอง** แบบเดียวกับ IC16 เพราะรันบน branch ที่เปิดไว้
     ตอน deploy สวิตช์เป็น false → เกณฑ์กลับด้านให้เอง
   ⚠ `OwlCoins.add/spend` และ `__housePaint` **ห้ามถูกถอด** — เกมจริงใช้อยู่
     (add = จ่ายรางวัลเควสต์ · __housePaint = โหมดช่างภาพของเด็ก) */
test('IM8: API สำหรับเทสผูกกับ DEV_API_ENABLED — ปิดแล้วต้องหายหมด แต่ของที่เกมใช้ต้องอยู่', async ({ page }) => {
  /* ⚠ ต้อง seed **ตัวละครในบ้าน** ด้วย ไม่งั้นปุ่ม "เข้าเมือง" ไม่โผล่
     (js/app-home2.js โชว์ปุ่มโหมดเฉพาะเด็กที่มี `char` แล้ว — คนที่ยังไม่มีเห็นปุ่มสร้างตัวละครแทน) */
  await page.addInitScript(id => {
    localStorage.setItem('p1quiz_house_' + id, JSON.stringify({
      v: 1, mapV: 4, tut: { skip: true },
      char: { gender: 0, hair: 0, hairC: 0, eyes: 1, eyeC: 0, shirt: 5, bottom: 0, shoes: 0 },
    }));
  }, 'icomain');
  const errs = await main(page);
  await page.locator('#child-select-view .child-card').first().click();
  const go = page.locator('.h2-mode-house, #house-entry-btn').first();
  if (!(await go.isVisible().catch(() => false))) test.skip(true, 'โหมดบ้านปิดอยู่');
  await go.click();
  await page.waitForFunction(() => !document.getElementById('house-view').hidden, null, { timeout: 60000 });
  await page.waitForTimeout(6000);

  const r = await page.evaluate(() => ({
    on:      !!window.__houseDbg,
    talkAt:  typeof window.__houseTalkAt === 'function',
    ...
    /* ต้องอยู่เสมอไม่ว่าสวิตช์จะเปิดหรือปิด */
    coinAdd: typeof (window.OwlCoins || {}).add === 'function',
    paint:   typeof window.__housePaint === 'function',
  }));

  expect(r.coinAdd, 'OwlCoins.add เป็นทางจ่ายรางวัลของเกมจริง ห้ามถอด').toBe(true);
  expect(r.paint, '__housePaint คือโหมดช่างภาพของเด็ก ห้ามถอด').toBe(true);

  if (r.on) {
    /* สวิตช์เปิด (branch พัฒนา) — ของเทสต้องครบ ไม่งั้นชุดเทสอื่นพังยกแผง */
    expect(r.talkAt && r.coinSet && r.shopDev && r.bookDev && r.playDev,
           'สวิตช์เปิดอยู่ ⇒ API ของเทสต้องครบทุกตัว').toBe(true);
  } else {
    /* สวิตช์ปิด (ค่าที่ต้องใช้ตอน deploy) — ทางโกงบรรทัดเดียวต้องหายหมด */
    expect(r.talkAt,  '__houseTalkAt ต้องถูกถอด').toBe(false);
    expect(r.coinSet, 'OwlCoins.set = ตั้งเงินได้ตามใจ ต้องถูกถอด').toBe(false);
    expect(r.shopDev, 'HouseShop.devUnlockAll = ปลดล็อกทั้งเกม ต้องถูกถอด').toBe(false);
    expect(r.bookDev, 'HouseBook.devMarkAll ต้องถูกถอด').toBe(false);
    expect(r.playDev, 'HousePlay.devGrow ต้องถูกถอด').toBe(false);
  }
  expect(errs).toEqual([]);
});
