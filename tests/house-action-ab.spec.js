...
test('🧭 เก็บของไปให้: มีลูกศรบอกทางไปของที่ต้องเก็บ · ของอยู่ในจอแล้วไม่ต้องมีลูกศรซ้ำ', async ({ page }) => {
  ...
  const r = await page.evaluate(async () => {
    ...
    /* เก็บครบตามที่สั่ง → ลูกศรต้อง **เปลี่ยนไปชี้ทางไปส่งงาน** ไม่ใช่หายไปเฉยๆ */
  ...
  /* ⚠ **เทสนี้ถูกกลับด้าน ไม่ได้ลบทิ้ง** (2026-08-22) — ของเดิมคาดว่า "เก็บครบแล้วลูกศรต้องหาย"
     ซึ่งเป็นพฤติกรรมก่อนรวมลูกศรเป็นตัวเดียว (ผู้ใช้สั่ง 2026-08-16 ว่า "แก้ลูกศรให้เอาจาก
     เควสต์ที่มีการนำทางมาใช้") ตอนนี้เก็บครบแล้วลูกศรต้องชี้ทางกลับไปส่งงานต่อ
     ไม่งั้นเด็กเก็บของครบแล้วยืนงงว่าต้องเอาไปให้ใคร */
  expect(r.done.target, 'เก็บครบแล้วลูกศรต้องชี้ทางไปส่งงานต่อ').toBeTruthy();
  expect(r.done.target.tile, 'ต้องมีช่องปลายทางจริง').toBeTruthy();
  expect(errs).toEqual([]);
});
