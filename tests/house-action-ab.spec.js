...
test('🧭 เก็บของไปให้: มีลูกศรบอกทางไปของที่ต้องเก็บ · ของอยู่ในจอแล้วไม่ต้องมีลูกศรซ้ำ', async ({ page }) => {
  ...
  /* ⚠ **เทสนี้ถูกกลับด้าน ไม่ได้ลบ (2026-08-22 · ผู้ใช้สั่ง)**
     เดิม: เก็บครบแล้วลูกศรหายไปเลย — เด็กต้องหาเองว่าเอาไปส่งใคร
     ตอนนี้: ลูกศรตัวเดียวกันนี้ถูกใช้กับ **เควสต์เดินทุกแบบ** ⇒ เก็บครบแล้วมันต้องหันไปชี้
             "คนที่ต้องเอาไปส่ง" ต่อทันที (นี่คือจังหวะที่เด็กต้องการคำแนะนำที่สุด) */
  expect(r.done.target, 'เก็บครบแล้วลูกศรต้องชี้ไปหาคนที่ต้องเอาไปส่ง').toBeTruthy();
  expect(errs).toEqual([]);
});
