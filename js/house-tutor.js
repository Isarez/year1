...
(function(){
  ...
  /* ⏱️ วาล์วนิรภัย: ขั้นเดียวห้ามค้างนานกว่านี้ (วินาที) — หมดเวลาแล้วข้ามไปเองเงียบๆ
     ⚠ ตัวเลขนี้ต้อง **เผื่อเด็กเดินข้ามเมืองจริง** (แผนที่ 56×42 เดินไกลสุด ~40 ช่อง ≈ 70 วิ)
       **และเผื่อขั้นที่ต้องเลือกของในร้าน** (เด็ก 5 ขวบเลือกเฟอร์นิเจอร์เป็นนาที)
       ตั้งสั้นกว่านี้แล้วบทเรียนจะข้ามตัวเองทั้งที่เด็กกำลังทำอยู่ */
  ...
  function markerAt(x, z){
    const w = W(); if(!w) return;
    markerClear();
    /* ⚠ วงแหวนอยู่ใน `worldGroup` ⇒ เข้าบ้านแล้วมันแค่ **ถูกซ่อน ไม่ได้ถูกลบ**
       ถ้าปั๊มใหม่ตอนอยู่ในบ้าน พอออกมาจะเห็น 2 วง (ค้างที่บ้าน + ที่จุดหมายจริง)
       — บั๊กที่ผู้ใช้แจ้ง 2026-08-17 */
    if(w.scene() !== 'out') return;
    const k = w.kit();
  ...
  function walkDist(t){
    ...
    if(!w || !g) return -1;
    /* ⚠ `pathLen` คิดบนกริดฉากนอกบ้านเสมอ — เอาพิกัดตอนอยู่ในบ้าน (กริดคนละใบ) ไปคิด
       จะได้ตัวเลขมั่วๆ แล้วขั้นเดินอาจ "จบเอง" ทั้งที่เด็กยังอยู่ในบ้าน */
    if(w.scene() !== 'out') return -1;
  ...
  function bindBtns(){
    ...
    if(e.go) e.go.addEventListener('click', ()=>{
      ...
      if(w.scene() !== 'out'){
        if(run) run.goPending = true;
        w.goOutside();
        w.toast('🚪', 'ออกไปหน้าบ้านก่อนนะ เดี๋ยวพาไปต่อเลย');
        return;
      }
  ...
  function nextChapterId(){
    ...
    /* ⚠ **ต้องเรียนเรียงตามลำดับ ห้ามข้ามบทที่ยังไม่พร้อมไปเรียนบทหลัง**
       ของเดิมใช้ `.find()` ที่กระโดดข้ามบทซึ่ง `ready()` ยังไม่ผ่าน ⇒ เด็กที่เงินไม่พอซื้อเมล็ด
       ตอนจบบท 1 จะโดนข้ามบทปลูกผักไปเรียนตกปลาเลย **แล้วไม่มีวันได้เรียนปลูกผักอีก**
       (ผู้ใช้แจ้ง 2026-08-17: "ไม่มีบทสอนไปซื้อเมล็ดพันธุ์และวิธีปลูกผัก")
       ⇒ ยังไม่พร้อม = **รอ** ไว้ก่อน เดี๋ยวเด็กหาเงินได้แล้วค่อยเริ่มเอง */
    const c = S.chapters.filter(x => !x.event && !chDone(x.id))[0];
    if(!c) return null;
    if(c.ready && !c.ready()) return null;
    return c.id;
  }
  ...
  function onScene(){
    /* ⚠ ต้อง **ลบของจริง** ไม่ใช่แค่ตั้ง null — ของอยู่ใน worldGroup ที่แค่ถูกซ่อนตอนเข้าบ้าน */
    markerClear();
    const s = step();
    const w = W();
    if(!s || !w || w.scene() !== 'out') return;
    const t = val(s.at);
    if(t) markerAt(t.x, t.z);
    /* 🚪 กด "พาไปเลย" ตอนอยู่ในบ้าน = ออกมาก่อน แล้วเดินต่อให้เองทันทีที่ถึงนอกบ้าน */
    if(run && run.goPending){
      run.goPending = false;
      const g = standTile(t);
      if(g) w.walkTo(g.x, g.z);
    }
