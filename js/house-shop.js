...
(function(){
  ...
  const STARTER_FURN = [
    ...
    'dining-table','chair',                 /* ครัว */
    'toilet',                               /* ห้องน้ำ */
    'tree','fence-corner','fence-seg','path','pet-house',   /* ของฉากนอกบ้าน */
  ];
  /* ตำแหน่งชุดมาตรฐานในบ้าน (พิกัดช่องกริดในบ้าน 20×14, anchor = มุมซ้ายบนของชิ้น, rot 0 = หันไป +z)
     ผังห้อง: z<=6 ครึ่งบน (x<=11 นั่งเล่น, x>=13 ครัว) · z>=8 ครึ่งล่าง (x<=13 นอน, x>=15 น้ำ)
     เลี่ยงแนวกำแพง (z=7, x=12 ครึ่งบน, x=14 ครึ่งล่าง) และช่องประตูหน้าบ้าน (x=4, z=0-1) */
  const STARTER_HOME = [
    {id:'sofa',         x:8,  z:2,  rot:0, col:0},
    {id:'coffee-table', x:8,  z:4,  rot:0, col:0},
    {id:'bookshelf',    x:10, z:1,  rot:0, col:0},
    {id:'dining-table', x:15, z:3,  rot:0, col:0},
    {id:'chair',        x:15, z:5,  rot:0, col:0},
    {id:'crib',         x:9,  z:10, rot:0, col:0},
    {id:'wardrobe',     x:11, z:12, rot:0, col:0},
    {id:'toilet',       x:17, z:10, rot:0, col:0},
  ];
  /* ชุดแต่งตัวที่ให้ฟรี — ทรงผม 2 · สีผม 1 · สีตา 1 · สีเสื้อ 4 · สีกางเกง 3 · สีรองเท้า 2
  ...
  const STARTER_FIT = {
    hair:[0,1], hairC:[0], eyeC:[0],
    shirt:[0,1,2,5], bottom:[0,1,2], shoes:[0,1],
