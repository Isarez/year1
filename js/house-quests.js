...
const FAM_BASE   = { A:6, B:8, C:7, board:8, family:10 };
  ...
  window.HOUSE_QUESTS = function(kit){
    ...
    /* ================= พูลกลไกของ NPC/กระดาน =================
       รวมทุกกลไกที่คนคนนี้แจกได้เป็นพูลเดียว แล้วสุ่ม **เฉลี่ยเท่ากัน** (ผู้ใช้สั่ง 2026-08-20)
       🔒 `quiz` ยังอยู่ในพูลของทุกคนเสมอ — ที่เปลี่ยนคือ "ความถี่" ไม่ใช่ "ความมีอยู่"
       🔒 **ห้ามฮาร์ดโค้ดเปอร์เซ็นต์รายกลไก** พูลโตทุกเฟส ตัวเลขตายตัวจะเพี้ยนเงียบๆ
          (เดิมล็อก quiz 40% ค้างมาตั้งแต่เฟส 5 จนกระดานเป็นการ์ดตอบคำถามกว่าครึ่ง) */
    /* น้ำหนักพิเศษ — ปกติ 1 · `deliver` ครึ่งเดียวเพราะเป็นงานเดินข้ามเมืองที่ไม่มีอะไรให้เล่นระหว่างทาง */
    ...
    /* กลไกที่จดไว้ตอนเช้าอาจเล่นไม่ได้แล้วตอนเปิดการ์ด (ของในโลกหมด · ขายเครื่องดนตรีทิ้ง)
       ⚠ **ห้ามตกกลับไปเป็น `quiz` เสมอ** — ยิ่งเล่นไปในวัน กระดานจะกลายเป็นตอบคำถามล้วน
         ⇒ สุ่มตัวแทนจากพูลของคนนั้น · **seed คงที่** เปิดกี่รอบก็ได้กลไกเดิม */
        ...
        gen(rng, diff){
          ...
          for(let r = 0; r < nR; r++){
            ...
            /* ราคาตามชั้น + ไล่ระดับในเควสต์ (ข้อแรกถูกสุด)
               ...
               📈 **ไล่ทีละชั้น ไม่ใช่ 3 ก้อน** (ผู้ใช้สั่ง 2026-08-17) — ของเดิมกระโดดจาก 24 (ป.4)
                 ไปเป็น 140 (ป.5) ทีเดียว ⇒ เด็ก ป.4 กับ ป.5 เจอโจทย์คนละโลกทั้งที่ห่างกันปีเดียว
                 index = tier (1 = ป.1 … 6 = ป.6) · เป็น lookup ตรงๆ ให้ปรับทีละชั้นได้ */
            const CAP  = [0, 9, 15, 30, 60, 100, 150];
            const cap  = CAP[Math.max(1, Math.min(6, diff.tier | 0))];
            const lo   = Math.max(2, Math.round(cap * .25));
            const price= Math.max(2, Math.round(lo + (cap - lo) * p));
            const good = pick(rng, PRICED_GOODS);
            /* 💵 **แบงก์ 20/50/100 โผล่เฉพาะตอนราคาถึง 20** (ผู้ใช้สั่ง 2026-08-17)
               ราคาน้อยยังเป็นเหรียญ 1/2/5/10 ล้วนเหมือนเดิม เด็กเล็กจะได้ไม่ต้องเรียนของใหม่
               ⚠ ไม่มีแบงก์แล้วราคาหลักร้อยจะต้องแตะเหรียญ 10 เป็นสิบครั้ง เด็กหยิบไม่ไหว
               ⚠ **ยื่นเฉพาะใบที่ไม่เกินราคา** — ของราคา 25 บาทแล้ววางแบงก์ 100 ให้เลือกด้วย
                 คือของที่หยิบมาก็จ่ายเกินอย่างเดียว เด็กเสียเวลาลองแล้วเจอ "เยอะไป" เปล่าๆ
                 (เกมร้านค้าของหน้าหลักใช้เกณฑ์เดียวกัน — ดู moneyUnitsFor ใน js/games-math.js) */
            const units = [1, 2, 5, 10].concat(price >= 20 ? [20, 50, 100].filter(n => n <= price) : []);
            out.push({kind:'coinpay', price: price, units: units,
    ...
    function martChangeMech(){
      return {
        ...
        gen(rng, diff, def){
          ...
          /* 💵 จ่ายด้วย "จำนวนที่จ่ายได้ด้วยแบงก์/เหรียญจริง" แล้วหน้าจอแตกเป็นกองให้เด็กนับเอง
             ⚠ ชั้นเล็กใช้จำนวนกลมเล็กๆ · ชั้นโตขยับเป็นหลักร้อยได้เพราะมีแบงก์ 100 แล้ว
               (ผู้ใช้สั่ง 2026-08-17: เกมทอนเงินต้องมีแบงก์ 20/50/100 ในตัวเลือกเสมอ)
             ⚠ **เดิมหยิบใบที่เล็กที่สุดที่จ่ายพอเสมอ (`notes[0]`) ⇒ ชั้นโตไม่ยากขึ้นเลย**
               ของในร้านแพงสุด 40 บาท ⇒ ป.5-6 ได้ scale [50,100,200] ก็จ่าย 50 ทุกครั้งอยู่ดี
               เงินทอนเลยอยู่แถว 10-40 เท่าชั้นเล็ก (ผู้ใช้แจ้ง 2026-08-17)
             ⇒ สุ่มจากใบที่ "จ่ายพอ **และ** ทอนไม่เกินเพดานของชั้น" แทน — ชั้นโตจึงเจอเงินทอน
               ก้อนใหญ่ที่ต้องแตกหลายใบจริงๆ · เพดานคุมไม่ให้กองเหรียญยาวเกินที่เด็กนับไหว */
          const scale = diff.tier <= 2 ? [10, 20, 30, 50]
                      : (diff.tier <= 4 ? [20, 50, 100] : [50, 100, 200]);
          const capChange = diff.tier <= 2 ? 20 : (diff.tier <= 4 ? 60 : 150);
          const over = scale.filter(v => v > item.price);
          const fit  = over.filter(v => v - item.price <= capChange);
          const paid = fit.length ? pick(rng, fit) : (over.length ? over[0] : item.price + 20);
    ...
    /* ---------- เฟส 13 · 🎨 ระบายสีตามเลข (ข้อ 52) ----------
       กริด pixel art มีเลขกำกับทุกช่อง · เลือกสีแล้วแตะช่องที่มีเลขตรงกัน
       🔒 **ไม่มีคำว่าผิด** — แตะช่องที่เลขไม่ตรงกับสีที่ถืออยู่ = ไม่มีอะไรเกิดขึ้น (ไม่นับพลาด)
          ระบายครบทุกช่อง = จบข้อ ⇒ เด็กทำเสร็จได้เสมอ ไม่มีทางตัน (กติกาเหล็กข้อ 1 + 2)
       🎁 รางวัลคือ **ภาพแขวนผนังจริง** ที่เอาไปติดในบ้านได้ (หน้าจอเป็นคนปลดล็อกให้)

       ⚠ กริดต้องเล็กพอสำหรับการ์ดใบเล็ก — 6×6 ถึง 8×8 เท่านั้น (12×12 ที่ IDEA.md เขียนไว้
         แตะไม่โดนบนมือถือ · บทเรียนเดียวกับเปียโนของเฟส 5) */
    const ART_PALETTE = [
      {n:1, c:'#E5533D', name:'แดง'},   {n:2, c:'#F0A14B', name:'ส้ม'},
      {n:3, c:'#F4C542', name:'เหลือง'}, {n:4, c:'#66BB6A', name:'เขียว'},
      {n:5, c:'#4FC3F7', name:'ฟ้า'},   {n:6, c:'#AB77DD', name:'ม่วง'},
    ];
    /* ภาพ pixel art — ตัวเลข = สีในจานข้างบน · 0 = ช่องว่าง (ไม่ต้องระบาย)
       ทุกภาพวาดให้ดูออกตั้งแต่ยังระบายไม่เสร็จ (เด็กจะได้รู้ว่ากำลังทำอะไรอยู่) */
    const ART_PICS = [
      {name:'ดอกไม้',   grid:['003300','035530','355553','035530','003300','000200']},
      {name:'บ้านหลังน้อย', grid:['001100','011110','111111','144441','144441','144441']},
      {name:'ปลาน้อย',  grid:['000000','005500','055555','555552','055550','000000']},
      {name:'ผีเสื้อ',   grid:['600006','661166','616616','661166','600006','000000']},
      {name:'ต้นไม้',   grid:['004400','044440','444444','044440','002200','002200']},
      {name:'หัวใจ',    grid:['011011','111111','111111','011110','001100','000000']},
      {name:'ดาวดวงน้อย', grid:['000300','003330','333333','033330','030030','000000']},
      {name:'เรือใบ',   grid:['000300','003330','033330','000300','111111','055550']},
      {name:'แมวน้อย',  grid:['200002','222222','212212','222222','022220','000000']},
      {name:'ร่มกันฝน',  grid:['055550','555555','000500','000500','000550','000000']},
      {name:'ลูกโป่ง',   grid:['011110','111111','111111','011110','000300','003000']},
      {name:'เห็ดน้อย',  grid:['011110','111111','011110','003300','003300','000000']},
    ];
    function colorNumMech(){
      return {
        id:'colornum', name:'ระบายสีตามเลข', fam:'D', shape:'drag', secPerQ: SEC_DRAG,
        gen(rng, diff){
          const out = [];
          /* ภาพเยอะเกินไปในเควสต์เดียวจะนาน — 2-3 ภาพต่อชุดกำลังดี */
          const nPic = diff.tier <= 2 ? 2 : 3;
          const pics = shuffled(rng, ART_PICS).slice(0, nPic);
          pics.forEach(pic=>{
            const cells = [];
            pic.grid.forEach((row, y)=>{
              Array.from(row).forEach((ch, x)=>{
                const n = ch | 0;
                if(n > 0) cells.push({x, y, n});
              });
            });
            /* จานสีเอาเฉพาะสีที่ภาพนี้ใช้จริง — มีสีที่ไม่ได้ใช้ = เด็กเสียเวลาลองผิด */
            const used = ART_PALETTE.filter(p => cells.some(c => c.n === p.n));
            out.push({kind:'colornum', pic:pic.name, w:pic.grid[0].length, h:pic.grid.length,
                      cells, palette:used, q:'ระบายสีตามเลขให้ครบทุกช่องนะ — ' + pic.name,
                      emoji:'🎨', choices:[], correct:0,
                      explain:'สวยมาก! เอาไปแขวนที่บ้านได้เลย 🖼️'});
          });
          return out;
        },
        /* payload = จำนวนช่องที่ระบายแล้ว — หน้าจอส่งมาเมื่อครบทุกช่องเท่านั้น */
        verify(it, done){ return {ok: (done | 0) >= it.cells.length}; },
      };
    }
    /* ---------- เฟส 13 · 🕵️ ทายว่าใคร (ข้อ 52) ----------
       **เปลี่ยนจากการ์ด 4 ตัวเลือกเป็นทรงเดิน** — โชว์เงาดำของชาวบ้าน แล้วเด็ก
       **เดินไปหาคนนั้นในเมืองแล้วแตะ** (ข้อ 52.2)
       เหตุผล: ถ้าทำเป็นการ์ดจะทับ `shadow` (ทายเงา) ที่เปิดใช้อยู่แล้ว = ของซ้ำ

       ⚠ ต้องเลือกเฉพาะคนที่เด็ก **คุยได้จริง** (`questableIds()`) ไม่งั้นเดินไปแล้วแตะไม่ติด
       ⚠ คำใบ้บอกได้แค่ **ย่าน** ห้ามบอกชื่อ (บอกชื่อ = เฉลยทันที) — หน้าจอเป็นคนเติมย่านให้
       ⚠ เงาวาดจากหน้าตาจริงของคนนั้น (`look` ใน NPC_DEFS) ⇒ ไฟล์นี้ส่งแค่ id ไป
         ให้ js/house.js เป็นคนวาด (ไฟล์นี้ไม่รู้จัก DOM/3D ตามกติกาเดิม) */
    function whoIsMech(){
      return {
        id:'whois', name:'ทายว่าใคร', fam:'B', walk:true,
        gen(rng, diff, def){
          ...
          return [{kind:'walk', target:'npc', toNpc: to, whois: true,
                   q:'ดูเงานี้สิ — คนนี้เป็นใครนะ? เดินไปหาให้เจอเลย!',
                   go:'ไปตามหา! 🕵️',
                   hint:'ตามหาคนที่เงาเหมือนภาพ',
                   emoji:'', choices:[], correct:0,
                   explain:'เจอตัวจริงแล้ว! ตาไวมากเลย'}];
        },
    ...
    const MECHS = {
      ...
      /* ---- 2026-08-15 — เปิด engine หน้าหลักที่ลงทะเบียนไว้แล้วแต่ไม่เคยถูกหยิบมาใช้ 12 ตัว ----
         ปัญหา: 26 จาก 49 กลไก (53%) เป็น "การ์ด 4 ตัวเลือก" ทั้งที่หน้าหลักมี engine พร้อมใช้
         อีก 12 ตัวนอนรออยู่ (ลงทะเบียน 23 · เปิดใช้แค่ 11)
         ⚠ **ทุกตัวผูกกับ NPC ที่เข้าธีมเท่านั้นผ่าน BONUS_MECHS — ไม่ใส่ใน ENGINE_MECHS**
           ถ้าโยนเข้าพูลรวม ชาวบ้านทั้งเมืองจะสุ่มได้หมด = โจทย์ไม่เข้ากับคนพูดเลย
           (ช่างไม้ชวนเล่นเศษส่วนพิซซ่า / แม่ค้าไอศกรีมชวนคำนวณพื้นที่)
         ⚠ `engineReady()` กรองระดับชั้นให้เองแล้ว — ตัวที่มีแต่หมวดชั้นสูงจะไม่ถูกแจกให้เด็กเล็ก
           (area/mirror ป.4-6 · world/coord ป.3-6) ⇒ ไม่ต้องกันเพิ่มที่นี่ */
      shopmoney: engineMech('money',    'ร้านค้านกฮูก',   '💰', {ask:'ช่วยคิดเงินให้ลูกค้าหน่อยได้ไหม?'}),
      slicefrac: engineMech('fraction', 'พิซซ่าเศษส่วน',  '🍕', {ask:'ช่วยแบ่งให้เท่าๆ กันหน่อยสิ!'}),
      caldays:   engineMech('calendar', 'ปฏิทิน',        '📅', {ask:'ช่วยดูปฏิทินให้หน่อยได้ไหม?'}),
      timeorder: engineMech('timeline', 'เส้นเวลา',       '⏳', {ask:'มาเรียงเรื่องราวตามเวลากันไหม?'}),
      chartread: engineMech('chart',    'แผนภูมิ',        '📊', {ask:'ช่วยอ่านแผนภูมินี้ให้หน่อยสิ!'}),
      globespin: engineMech('world',    'หมุนโลก',        '🌍', {ask:'อยากรู้จักที่ไกลๆ ไหม? มาหมุนลูกโลกกัน!'}),
    ...
    /* 🔒 **แหล่งเดียวของไอคอนกลไก** — เพิ่มกลไกใหม่ต้องเติมที่นี่ (เทส QT4 ไล่ตรวจว่าครบ)
       เดิมอยู่ใน `MECH_TABS` ของ house-qbrowse.js ซึ่งปิดตอน deploy ⇒ เกมจริงมองไม่เห็น (ย้าย 2026-08-23) */
    const MECH_IC = {
    ...
    const LAB_MECHS = {
      /* 🚫 เคยจะเอา engine "นักวิทยาศาสตร์" ของหน้าหลักมาลงที่ ดร.ฟ้า แต่**เปิดไม่ได้** —
         view นั้นเป็นหน้าเต็มจอที่มีพื้นหลังของตัวเอง เอามาแปะในการ์ดแล้วเนื้อหาตก 199px
         (ดูหมายเหตุใน ALLOW ของ js/house-games.js) ⇒ งานวิทย์ที่แล็บยังใช้ของที่เขียนเองในเฟส 6 */
    ...
    const BONUS_MECHS = [
      ...
      /* 🕵️ ทายว่าใคร (เฟส 13 · ทรงเดิน) — คนที่รู้จักคนทั้งเมือง: ผู้ใหญ่บ้าน/เทศมนตรี/ไปรษณีย์/ตำรวจ */
      [/^npc-(headman|mayor|post|police|granny)/, ['whois']],
      /* 🎨 ระบายสีตามเลข (เฟส 13) — ร้านของเล่น/เด็กๆ/ตัวตลก/นักมายากล (สายศิลปะ-สร้างสรรค์) */
      [/^npc-(toy|kid|play|clown|magician|garden)/, ['colornum']],
      ...
      /* ---- 2026-08-15: engine หน้าหลักอีก 12 ตัว — ผูกกับ "คนที่โจทย์นั้นเข้ากับอาชีพเขา" ----
         ...
         ⚠ ทุกแถวเป็น "เพิ่มเข้าพูล" ไม่ใช่ "แทนที่พูล" ⇒ NPC พวกนี้ยังได้ quiz/count/เกมเดิมตามปกติ */
      /* 💰 ร้านค้าทุกชนิด = คิดเงินหน้าร้านจริง (คู่กับ payexact/changeback ที่มีอยู่แล้ว) */
      ...
      /* 📊 ครู/นักเรียน + ร้านใหญ่ที่ต้องดูยอดขาย = อ่านแผนภูมิ */
      [/^npc-(teacher|stu|student|mart|mall)/, ['chartread']],
      /* 🌍 นักเดินทาง/โรงแรม/ชายหาด = โลกกว้าง */
      [/^npc-(traveler|hotel|beach)/,      ['globespin']],
      /* 🚧 พิกัด (ตำรวจ/ไปรษณีย์/ชาวประมง) + พื้นที่ (ช่างไม้/ชาวนา/ร้านเฟอร์ฯ) + ลากเส้นต่อจุด (เด็กๆ)
         เตรียมที่ทางไว้แล้วแต่ **ยังเปิดไม่ได้** — เนื้อหาตกใต้ขอบการ์ดบนจอเตี้ย
         (เหตุผลเต็มอยู่ที่ ALLOW ใน js/house-games.js) */
      /* 🪞 ร้านเสื้อผ้า (มีกระจกลองชุด) + นักมายากล/ตัวตลก = ภาพสะท้อน */
      [/^npc-(mall-fash|magician|clown)/,  ['mirrorsym']],
      /* ✏️🦉 เด็กๆ/สนามเด็กเล่น/ร้านของเล่น = เกมเล่นด้วยกัน (ลากเส้นต่อจุดมีแต่หมวด ป.1) */
      [/^npc-(kid|play|toy|clown)/,        ['owlsay']],
      [/^npc-(stu|student|granny)/,        ['owlsay']],
    ];
