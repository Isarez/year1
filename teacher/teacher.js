...
/* ---------- mechanics — เปิดครบทุกรูปแบบ (quiz + AR 4 แบบ + ฟังคำศัพท์ + เกมฝึกทักษะ 3 แบบ)
   icon = SVG หัว section บนหน้า home (ชุดเดียวกับหน้าหลัก), form = รูปแบบฟอร์มใส่โจทย์
   group = หมวดใหญ่สำหรับจัดกลุ่มบนหน้าเลือกสร้างเกม (ล้อกับหน้าหลักเด็ก) ---------- */
const MECH_GROUPS = [
  {id:'quiz',   name:'ถาม-ตอบ',        icon:'../assets/icons/section-quiz.svg'},
  {id:'ar',     name:'การโต้ตอบ (AR)',  icon:'../assets/icons/section-ar.svg'},
  {id:'skill',  name:'เกมฝึกทักษะ',     icon:'../assets/icons/section-skill.svg'},
  {id:'listen', name:'ฟังคำศัพท์',      icon:'../assets/icons/section-listen.svg'}
];
const MECHANICS = [
  {id:'quiz',        name:'ถาม-ตอบปรนัย', emoji:'📝', enabled:true,  form:'choices',  group:'quiz',   icon:'../assets/icons/section-quiz.svg'},
  {id:'ar-pick',     name:'AR หยิบการ์ดคำตอบที่ถูกต้อง', emoji:'🖐️', enabled:true,  form:'choices',  group:'ar',     icon:'../assets/icons/section-ar.svg'},
  {id:'ar-sentence', name:'AR หยิบการ์ดเรียงลำดับ', emoji:'🔤', enabled:true,  form:'sentence', group:'ar',     icon:'../assets/icons/section-ar.svg'},
  {id:'ar-connect',  name:'AR โยงเส้น', emoji:'🪢', enabled:true,  form:'pairs',    group:'ar',     icon:'../assets/icons/section-ar.svg'},
  {id:'ar-count',    name:'AR หยิบให้ครบ', emoji:'🧺', enabled:true,  form:'count',    group:'ar',     icon:'../assets/icons/section-ar.svg', desktopOnly:true},
  {id:'listen',      name:'ฟังคำศัพท์ สะกดคำ', emoji:'🎧', enabled:true,  form:'words',    group:'listen', icon:'../assets/icons/section-listen.svg'},
  {id:'match',       name:'จับคู่ความจำ', emoji:'🎴', enabled:true,  form:'pairs',    group:'skill',  icon:'../assets/icons/section-skill.svg'},
  {id:'mix',         name:'ผสมสี', emoji:'🎨', enabled:true,  form:'mix',      group:'skill',  icon:'../assets/icons/section-skill.svg'},
  {id:'music',       name:'ดนตรี', emoji:'🎹', enabled:true,  form:'music',    group:'skill',  icon:'../assets/icons/section-skill.svg'}
];
function mechById(id){ return MECHANICS.find(m=>m.id===id) || MECHANICS[0]; }
/* หน่วยนับจำนวนโจทย์/ด่านต่อรอบ ตามชนิดเกม (โชว์บนการ์ด/หน้าจัดการ) */
function mechCountUnit(mechId){
  if(mechId==='match') return 'คู่';
  if(mechId==='mix')   return 'สูตร';
  if(mechId==='music') return 'ทำนอง';
  const form = mechById(mechId).form;
  return (form==='pairs' || form==='words') ? 'ด่าน' : 'ข้อ/รอบ';
}
/* คำนามเรียกโจทย์ 1 หน่วยของเกม (ใช้ในหน้าจัดการ: "X คู่/รอบ (มี Y คู่)") */
function mechItemWord(mechId){
  if(mechId==='match') return 'คู่';
  if(mechId==='mix')   return 'สูตร';
  if(mechId==='music') return 'ทำนอง';
  return mechById(mechId).form==='words' ? 'คำ' : 'ข้อ';
}
...
const mixView     = $('mix-view');
const musicView   = $('music-view');
const resultView  = $('result-view');
function showView(v){
  [setupView, homeView, manageView, builderView, quizView, listenView, memoryView, mixView, musicView, resultView].forEach(x=>{ x.hidden = (x!==v); });
  const av = $('ar-view');
  if(av) av.hidden = true; /* ar-view จัดการแยกใน startTeacherAR (teacher-ar.js) — ที่นี่แค่กันซ้อน */
  window.scrollTo({top:0, behavior:'smooth'});
}
let lastPlay = null;   // {type:'quiz'|'ar'|'listen'|'match'|'mix'|'music', gameId} — ให้ปุ่ม "ทำอีกครั้ง" รู้ว่าเล่นเกมไหน/engine ไหน
...
/* column เกมของชั้นที่เลือก จัดกลุ่มเป็นหมวดใหญ่ (ถาม-ตอบ / การโต้ตอบ AR / เกมฝึกทักษะ / ฟังคำศัพท์)
   ล้อกับหน้าหลักเด็ก + หน้าเลือกสร้างเกม — หัว section ใช้ icon+ชื่อหมวด, ในหมวดเรียงเกมตามชนิด (mechanic)
   fade-in ไล่จังหวะตอนสลับ filter */
function renderMechGroups(published){
  const wrap = $('teacher-mech-groups');
  wrap.innerHTML = '';
  const list = published.filter(g=>g.grade===selectedGrade);
  const mechOrder = id => MECHANICS.findIndex(m=>m.id===id);
  let seq = 0;
  MECH_GROUPS.forEach(gr=>{
    const games2 = list.filter(g=>mechById(g.mechanic).group===gr.id)
      .sort((a,b)=>mechOrder(a.mechanic)-mechOrder(b.mechanic)); /* เกมชนิดเดียวกันอยู่ติดกันในหมวด */
    if(!games2.length) return;
    const title = document.createElement('div');
    title.className = 'teacher-grade-title tg-fade';
    title.style.animationDelay = (seq*0.05)+'s'; seq++;
    title.innerHTML = '<img src="'+gr.icon+'" class="cst-icon" alt=""> '+gr.name;
...
function buildGameCard(game){
  ...
  card.innerHTML =
    ...
    '<div class="cat-meta">'+game.questionCount+' '+mechCountUnit(game.mechanic)+'</div>';
...
function playTeacherGame(gameId){
  ...
  else if(game.mechanic==='match') startTeacherMatch(gameId);
  else if(game.mechanic==='mix') startTeacherMix(gameId);
  else if(game.mechanic==='music') startTeacherMusic(gameId);
    ...
    row.innerHTML =
      ...
      '<div class="mg-info">'+
        ...
        '<div class="mg-meta"><img src="'+mech.icon+'" class="mg-mech-icon" alt=""> '+mech.name+' · '+game.questionCount+' '+
          mechItemWord(game.mechanic)+'/รอบ (มี '+game.questions.length+' '+mechItemWord(game.mechanic)+')</div>'+
      '</div>'+
...
/* เลือกรูปแบบเกม — จัดกลุ่มเป็นหมวด (ถาม-ตอบ / การโต้ตอบ AR / เกมฝึกทักษะ / ฟังคำศัพท์)
   แต่ละหมวดมีหัวข้อ (icon+ชื่อ) คั่นก่อนกลุ่มชิป ให้ล้อกับหน้าหลักเด็ก */
function renderMechanicPicker(){
  const wrap = $('mechanic-picker');
  wrap.innerHTML = '';
  MECH_GROUPS.forEach(gr=>{
    const mechs = MECHANICS.filter(m=>m.group===gr.id);
    if(!mechs.length) return;
    const title = document.createElement('div');
    title.className = 'mech-group-title';
    title.innerHTML = '<img src="'+gr.icon+'" class="mech-group-icon" alt="">'+gr.name;
    wrap.appendChild(title);
    const row = document.createElement('div');
    row.className = 'mech-group-row';
    mechs.forEach(m=>{
      const b = document.createElement('button');
      b.type = 'button';
...
function renderQuestionArea(){
  ...
  $('bq-label-sub').textContent =
    ...
    form==='mix'      ? '(กำหนดสีที่ต้องผสม 2-3 สี + สีผลลัพธ์ที่จะได้ ตั้งชื่อสีเองได้ ตอนเล่นเด็กหยอดสีให้ครบตามสูตร)' :
    form==='music'    ? '(แต่งทำนองด้วยการกดเปียโนเรียงเป็นลำดับโน้ต ตอนเล่นเด็กจะฟังแล้วกดคีย์ตาม)' :
    selectedMechanic==='match'
                      ? '(ใส่คู่ที่ตรงกัน ซ้าย ↔ ขวา ตอนเล่นเด็กจะเปิดการ์ดจับคู่ให้ค่าตรงกัน)' :
                        '(ใส่คู่ที่ตรงกัน ซ้าย ↔ ขวา ตอนเล่นจะสุ่มมาโยงครั้งละไม่เกิน 4 คู่)';
  $('b-count-note').textContent =
    selectedMechanic==='match'
      ? '💡 จำนวนนี้คือจำนวนคู่การ์ดต่อรอบเล่น — ใส่คู่ได้มากกว่านี้ ระบบจะสุ่มคู่มาให้เล่นไม่ซ้ำ'
      : form==='pairs'
      ...
      : form==='mix'
      ? '💡 จำนวนนี้คือจำนวนด่านต่อรอบเล่น (1 สูตร = 1 ด่าน) — ใส่สูตรมากกว่านี้ได้ ระบบจะสุ่มมาให้ทุกรอบเล่น'
      : form==='music'
      ? '💡 จำนวนนี้คือจำนวนด่านต่อรอบเล่น (1 ทำนอง = 1 ด่าน) — ใส่ทำนองมากกว่านี้ได้ ระบบจะสุ่มมาให้ทุกรอบเล่น'
...
function buildQuestionBlock(qData){
  ...
  if(form==='mix')      return buildMixBlock(qData);
  if(form==='music')    return buildMusicBlock(qData);
...
/* ฟอร์มผสมสี (mix): 1 ข้อ = 1 สูตร — สีที่ต้องผสม 2-3 สี (เลือกสี+ตั้งชื่อ) + สีผลลัพธ์ (เลือกสี+ตั้งชื่อ)
   สีเก็บเป็น hex, ชื่อครูตั้งเอง — ตอนเล่นโชว์สีผลลัพธ์ที่ครูตั้งตรงๆ ไม่คำนวณผสม RGB */
function buildMixBlock(qData){
  const block = bqShell(
    '<div class="bq-mix">'+
      '<div class="bq-mix-label">🎨 สีที่ต้องผสม (2-3 สี)</div>'+
      '<div class="bq-mix-ings"></div>'+
      '<button type="button" class="bq-add-ans-btn bq-mix-add">＋ เพิ่มสีที่ต้องผสม</button>'+
      '<div class="bq-mix-label bq-mix-result-label">✨ สีผลลัพธ์ที่จะได้</div>'+
      '<div class="bq-mix-row bq-mix-result">'+
        '<input type="color" class="bq-color-input bq-mix-rcolor" value="#FB8C00" title="เลือกสีผลลัพธ์">'+
        '<input class="child-name-input bq-ans-input bq-mix-rname" type="text" placeholder="ชื่อสีผลลัพธ์ เช่น สีส้ม" maxlength="30">'+
      '</div>'+
    '</div>');
  const ings = block.querySelector('.bq-mix-ings');
  const DEF_COLORS = ['#E53935','#FDD835','#1E63C4','#43A047','#8E44AD','#FB8C00'];
  function addIng(c, n){
    const row = document.createElement('div');
    row.className = 'bq-mix-row bq-mix-ing';
    row.innerHTML =
      '<input type="color" class="bq-color-input bq-mix-icolor" title="เลือกสี">'+
      '<input class="child-name-input bq-ans-input bq-mix-iname" type="text" placeholder="ชื่อสี เช่น แดง" maxlength="30">'+
      '<button type="button" class="bq-ans-del" title="ลบสีนี้">✕</button>';
    row.querySelector('.bq-mix-icolor').value = c || DEF_COLORS[ings.children.length % DEF_COLORS.length];
    row.querySelector('.bq-mix-iname').value = n || '';
    row.querySelector('.bq-ans-del').addEventListener('click', ()=>{
      playClick();
      if(ings.children.length <= 2){ showToast('⚠️','ต้องมีสีที่ต้องผสมอย่างน้อย 2 สีนะคะ'); return; }
      row.remove();
    });
    ings.appendChild(row);
  }
  block.querySelector('.bq-mix-add').addEventListener('click', ()=>{
    playClick();
    if(ings.children.length >= 3){ showToast('⚠️','ผสมได้สูงสุด 3 สีต่อสูตรนะคะ'); return; }
    addIng('', '');
  });
  if(qData){
    (qData.ingredients||[]).forEach(g=>addIng(g.c, g.n));
    while(ings.children.length < 2) addIng('', '');
    block.querySelector('.bq-mix-rcolor').value = qData.result.c;
    block.querySelector('.bq-mix-rname').value = qData.result.n;
  } else {
    addIng('#E53935','แดง'); addIng('#FDD835','เหลือง');
  }
  return block;
}
/* ฟอร์มดนตรี (music): 1 ข้อ = 1 ทำนอง — กดเปียโนจิ๋วเรียงเป็นลำดับโน้ต (2-30 โน้ต) เก็บเป็น index คีย์ขาว
   MUSIC_WHITE_KEYS/musicKeyLabel/playPianoNote/playMusicSequence มาจาก teacher-music.js (โหลดทีหลัง เรียกตอน runtime จึงหาเจอ) */
function buildMusicBlock(qData){
  const block = bqShell(
    '<div class="bq-music">'+
      '<div class="bq-music-label">🎹 กดคีย์เปียโนเรียงเป็นทำนอง (2-30 โน้ต)</div>'+
      '<div class="bq-music-notes" aria-live="polite"></div>'+
      '<div class="bq-music-tools">'+
        '<button type="button" class="bq-music-btn bq-music-play">▶ ลองฟัง</button>'+
        '<button type="button" class="bq-music-btn bq-music-undo">⌫ ลบตัวท้าย</button>'+
        '<button type="button" class="bq-music-btn bq-music-clear">🗑 ล้าง</button>'+
      '</div>'+
      '<div class="bq-music-piano"></div>'+
    '</div>');
  const seq = (qData && qData.notes) ? qData.notes.slice() : [];
  block._musicSeq = seq;
  const notesWrap = block.querySelector('.bq-music-notes');
  const pianoWrap = block.querySelector('.bq-music-piano');
  function renderNotes(){
    notesWrap.innerHTML = '';
    if(!seq.length){
      const empty = document.createElement('span');
      empty.className = 'bq-music-empty';
      empty.textContent = 'ยังไม่มีโน้ต — กดคีย์ด้านล่างเพื่อเพิ่มทำนอง';
    ...
    seq.forEach(wi=>{
      ...
      chip.className = 'bq-music-chip';
      chip.style.setProperty('--key-color', k.color);
      chip.textContent = musicKeyLabel(k);
  ...
  MUSIC_WHITE_KEYS.forEach((k,i)=>{
    ...
    key.type = 'button';
    key.className = 'bq-music-key';
    key.style.setProperty('--key-color', k.color);
    key.innerHTML = '<span>'+musicKeyLabel(k)+'</span>';
    key.addEventListener('click', ()=>{
      if(seq.length >= 30){ showToast('⚠️','ทำนองยาวได้สูงสุด 30 โน้ตนะคะ'); return; }
      playClick();
      playPianoNote(k.freq, 0.7);
      seq.push(i);
      renderNotes();
    });
    pianoWrap.appendChild(key);
  });
  block.querySelector('.bq-music-play').addEventListener('click', ()=>{
    playClick();
    if(!seq.length){ showToast('🎵','ยังไม่มีโน้ตให้ฟังเลยค่ะ'); return; }
    playMusicSequence(seq, true); /* noFlash — ฟอร์มไม่มีคีย์เกมให้ไฮไลต์ */
  });
  block.querySelector('.bq-music-undo').addEventListener('click', ()=>{ playClick(); seq.pop(); renderNotes(); });
  block.querySelector('.bq-music-clear').addEventListener('click', ()=>{ playClick(); seq.length = 0; renderNotes(); });
  renderNotes();
  return block;
}
...
function collectBuilderData(){
  ...
  for(let i=0;i<blocks.length;i++){
    ...
    if(form==='sentence'){
      ...
      const ingRows = Array.from(b.querySelectorAll('.bq-mix-ings .bq-mix-ing'));
      const ingredients = [];
      for(const r of ingRows){
        const c = r.querySelector('.bq-mix-icolor').value;
        const n = r.querySelector('.bq-mix-iname').value.trim();
        if(!n){ showToast('✏️','สูตรที่ '+(i+1)+' มีสีที่ยังไม่ได้ตั้งชื่อนะคะ'); return null; }
        ingredients.push({ c, n });
      }
      if(ingredients.length < 2){ showToast('⚠️','สูตรที่ '+(i+1)+' ต้องมีสีที่ต้องผสมอย่างน้อย 2 สีนะคะ'); return null; }
      const rc = b.querySelector('.bq-mix-rcolor').value;
      const rn = b.querySelector('.bq-mix-rname').value.trim();
      if(!rn){ showToast('✨','สูตรที่ '+(i+1)+' ยังไม่ได้ตั้งชื่อสีผลลัพธ์นะคะ'); return null; }
      questions.push({ ingredients, result:{ c:rc, n:rn } });
    } else if(form==='music'){
      const seq = (b._musicSeq || []).slice();
      if(seq.length < 2){ showToast('🎵','ทำนองที่ '+(i+1)+' ต้องมีอย่างน้อย 2 โน้ตนะคะ'); return null; }
      if(seq.length > 8){ showToast('⚠️','ทำนองที่ '+(i+1)+' ยาวเกิน 8 โน้ตนะคะ'); return null; }
      questions.push({ notes: seq });
    } else {
      const qText = b.querySelector('.bq-q-input').value.trim();
  ...
  /* โยงเส้น/จับคู่ความจำ: ต้องมีอย่างน้อย 2 คู่ */
  if(form==='pairs' && questions.length < 2){ showToast('⚠️', (selectedMechanic==='match'?'เกมจับคู่ความจำ':'เกมโยงเส้น')+'ต้องมีอย่างน้อย 2 คู่นะคะ'); return null; }
  const data = {
    grade: $('b-grade').value,
...
/* ผลลัพธ์เกมฝึกทักษะ (จับคู่/ผสมสี/ดนตรี) — ดาวตาม mistakes เหมือน AR, ไม่มี review, ไม่มีสติกเกอร์
   engine ใน teacher-match.js/teacher-mix.js/teacher-music.js เรียกใช้ร่วมกัน */
function showTeacherSkillResult(stars, scoreLine){
  showView(resultView);
  $('result-emoji').textContent = stars===3 ? '🏆' : stars===2 ? '🎉' : '💪';
  $('result-title').textContent = stars===3 ? 'สุดยอดไปเลย!' : stars===2 ? 'เก่งมากเลย!' : 'ทำได้ดีแล้วนะ!';
  const starsRow = $('stars-row');
  ...
  $('score-line').textContent = scoreLine;
  $('score-sub').textContent = stars===3 ? 'เก่งสุด ๆ ไปเลย!' : stars===2 ? 'เก่งขึ้นทุกวันเลยนะ!' : 'ไม่เป็นไรนะ ลองอีกครั้งได้เสมอ!';
...
$('retry-btn').addEventListener('click', ()=>{
  ...
  else if(lastPlay.type==='match') startTeacherMatch(lastPlay.gameId);
  else if(lastPlay.type==='mix') startTeacherMix(lastPlay.gameId);
  else if(lastPlay.type==='music') startTeacherMusic(lastPlay.gameId);
